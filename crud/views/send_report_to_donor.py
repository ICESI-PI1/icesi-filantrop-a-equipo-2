...
def send_report_to_donor(request):
    if request.method == 'POST':
        try:
            ...
            try:
                crea_assistance = CREAReport.objects.get(
                    student_code=selected_student_id)
            ...
            report_name = generate_general_report(
                date, semester, selected_student, student_testimony, non_academic_activities, crea_assistance)
...
def send_report(request):
    try:
        ...
        send_email(selected_donor.email, email_message,
                   subject, attachment_path=report_path)
...
def generate_general_report(date, semester, student, testimony, non_academic_activities, crea_assistance):
    try:
        ...
        report_name = "Reporte general {} - {}.docx".format(
            student.student_code, semester)
        ...
        for paragraph_info in format_content:
            ...
            paragraph_text = paragraph_text.replace(
                "[testimonio_estudiante]", testimony)
            paragraph_text = paragraph_text.replace(
                "[actividades_no_académicas]", str(non_academic_activities))
            paragraph_text = paragraph_text.replace(
                "[asistencia_monitorias]", str(crea_assistance))
