...
class UploadFileTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.url = reverse('uploadFile')  # Asegúrate de usar la URL correcta según tu configuración de URLs
    ...
    def test_post_no_file(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'upload_academic_report.html')
        self.assertEqual(response.context['result_message'], 'No se proporcionó ningún archivo')
        self.assertQuerysetEqual(response.context['files'], [])
    ...
    def test_post_student_not_found(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'upload_academic_report.html')
        self.assertIn('Estudiante no encontrado', response.context['result_message'])
        self.assertQuerysetEqual(response.context['files'], [])

    def test_post_successful_upload(self):
        # Asegúrate de que el código del estudiante aquí exista en la base de datos
        student = Student.objects.create(student_code='codigo_estudiante_existente')
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'upload_academic_report.html')
        self.assertEqual(response.context['result_message'], 'Archivo cargado exitosamente')
        self.assertQuerysetEqual(response.context['files'], [repr(Document.objects.get(uploadedFile='archivo.txt'))])
